...
class MDXCSeparator(CommonSeparator):
    ...
    def demix(self, mix: np.ndarray) -> dict:
        ...
        if self.is_roformer:
            mix = torch.tensor(mix, dtype=torch.bfloat16)
            ...
            window = torch.tensor(signal.windows.hamming(chunk_size), dtype=torch.bfloat16)
            ...
            with torch.no_grad():
                ...
                result = torch.zeros(req_shape, dtype=torch.bfloat16).to(device)
                counter = torch.zeros(req_shape, dtype=torch.bfloat16).to(device)
